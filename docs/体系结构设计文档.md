...
数据层主要给业务逻辑层提供数据访问服务，包括对于持久化数据的增、删、改、查。例如，业务逻辑模块userbl需要的服务由userdataservice接口提供。userdataservice接口通过mybatis绑定同名xml文件实现。持久化数据的保存通过mysql数据库的形式。数据层模块的描述具体设计如图13所示。
...
<div style="text-align: center">图15 activitydata模块设计图</div>

![](https://s2.ax1x.com/2019/06/05/VNqcuT.png)

<div style="text-align: center">图16 carddata模块设计图</div>

![](https://s2.ax1x.com/2019/06/05/VNqgDU.png)

<div style="text-align: center">图17 coupondata模块设计图</div>
